...
import { 
  FiUsers, FiUserCheck, FiUserPlus, FiActivity,
  FiCalendar, FiMail, FiPhone, FiMapPin,
  FiDownload, FiFilter, FiSearch, FiEye,
  FiBarChart2, FiTrendingUp, FiClock,
  FiAward, FiBookOpen, FiSmartphone
} from 'react-icons/fi';
import { 
  FaChartLine, FaUsers, FaUserGraduate, 
  FaRegCalendarAlt, FaRegClock, FaMobileAlt
} from 'react-icons/fa';
...
function UserAnalytics() {
  ...
  useEffect(() => {
   ...
   const breadcrumbPaths = [
    ...
    { name: 'Analytics', href: '/admin/users/analytics' } // Ye current page hai, automatically blue aur unclickable ho jayega
  ];
  ...
  const fetchUserData = async () => {
    try {
      ...
      // Device type distribution (mock for now - can be extended)
      const deviceDist = {
        'Mobile': Math.floor(usersData.length * 0.65),
        'Desktop': Math.floor(usersData.length * 0.30),
      ...
      setStats({
        ...
        loginHistory: usersData.slice(0, 10) // Recent users
      });
                  ...
                  <div className="flex-1 bg-gray-800 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full ${
                        ...
                        device === 'Desktop' ? 'bg-blue-500' : 'bg-purple-500'
                      }`}
                      style={{ width: `${(count / stats.totalUsers) * 100}%` }}
                    />
                  </div>
        ...
        >
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-glass-bg border border-glass-border rounded-xl py-3 pl-12 pr-4 text-white placeholder-gray-400 focus:outline-none focus:border-green-500/50"
            />
          </div>
          
          <button
            onClick={exportToCSV}
            className="flex items-center gap-2 px-6 py-3 bg-glass-bg border border-glass-border rounded-xl hover:border-green-500/50 transition-all"
          >
              ...
              <tbody>
                {filteredUsers.map((user, index) => (
                  <motion.tr
                    ...
                    className="border-b border-glass-border last:border-0 hover:bg-white/5 transition-all"
                  >
        ...
        {selectedUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-deep-black border border-glass-border rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"
            >
              <div className="p-6">
