      ...
      <div className='ml-0 min-h-screen p-6'>
        <div className='max-w-7xl mx-auto space-y-8'>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className='text-2xl md:text-3xl font-semibold text-slate-50 mb-1'>Test Management</h1>
          ...
          <div className='flex flex-col md:flex-row gap-4 items-center justify-between bg-slate-900 p-4 rounded-xl border border-slate-800'>
            <div className='flex flex-1 gap-4 w-full'>
              <div className='relative flex-1 max-w-md'>
                ...
                <input type='text' placeholder='Search tests...' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className='w-full bg-slate-950 border border-slate-800 rounded-lg py-2 pl-10 pr-4 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all' />
              </div>

              {/* Custom Dropdown for Categories */}
              <div className='relative min-w-[200px]'>
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className='w-full flex items-center justify-between bg-slate-950 border border-slate-800 rounded-lg py-2 px-4 text-sm text-slate-200 hover:border-slate-700 transition-all'>
                  <span className='truncate'>{selectedCategory === "all" ? "All Categories" : getCategoryName(selectedCategory)}</span>
                  <FiChevronDown className={`transition-transform duration-200 ${isDropdownOpen ? "rotate-180" : ""}`} />
                </button>

                <AnimatePresence>
                  {isDropdownOpen && (
                    <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className='absolute top-full mt-2 w-full bg-slate-900 border border-slate-800 rounded-lg shadow-xl z-20 overflow-hidden'>
                  ...
                  {filteredTests.length === 0 ? (
                    ...
                    filteredTests.map((test) => (
                      <tr key={test.id} className='hover:bg-slate-800/30 transition-colors'>
                        ...
                        <td className='py-3 px-4'>
                          <span className='inline-flex items-center px-2 py-1 rounded-md bg-slate-800 text-slate-300 text-xs'>{getCategoryName(test.category_id)}</span>
                        </td>
                        <td className='py-3 px-4'>
                          <span className='inline-flex items-center px-2 py-1 rounded-md bg-slate-800 text-indigo-300 text-xs border border-indigo-500/20'>{getExamName(test.exam_id)}</span>
                        </td>
                        ...
                        <td className='py-3 px-4 text-slate-400'>{test.total_questions || 0} Qs</td>
                        <td className='py-3 px-4'>
                          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${test.status === "published" ? "bg-emerald-500/10 text-emerald-400" : test.status === "draft" ? "bg-amber-500/10 text-amber-400" : "bg-slate-500/10 text-slate-400"}`}>{test.status || "draft"}</span>
                        </td>
          ...
          <AnimatePresence>
            {editingTest && (
              <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm'>
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className='bg-slate-900 border border-slate-800 rounded-xl max-w-2xl w-full shadow-2xl'>
                  ...
                  >
                    <div>
                      ...
                      <input type='text' value={editingTest.title} onChange={(e) => setEditingTest({ ...editingTest, title: e.target.value })} className='w-full bg-slate-950 border border-slate-800 rounded-lg py-2 px-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500' />
                    </div>
                    <div>
                      <label className='block text-sm font-medium text-slate-300 mb-1.5'>Description</label>
                      <textarea rows='3' value={editingTest.description} onChange={(e) => setEditingTest({ ...editingTest, description: e.target.value })} className='w-full bg-slate-950 border border-slate-800 rounded-lg py-2 px-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500' />
                    </div>
                    <div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
                      <div>
                        <label className='block text-sm font-medium text-slate-300 mb-1.5'>Category</label>
                        <select value={editingTest.category_id} onChange={(e) => setEditingTest({ ...editingTest, category_id: e.target.value })} className='w-full bg-slate-950 border border-slate-800 rounded-lg py-2 px-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500'>
                          {categories.map((cat) => (
                      ...
                      <div>
                        ...
                        <select value={editingTest.exam_id} onChange={(e) => setEditingTest({ ...editingTest, exam_id: e.target.value })} className='w-full bg-slate-950 border border-slate-800 rounded-lg py-2 px-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500'>
                          {exams
                            .filter((ex) => String(ex.category_id) === String(editingTest.category_id))
                            .map((exam) => (
                      ...
                      <div>
                        ...
                        <select value={editingTest.status} onChange={(e) => setEditingTest({ ...editingTest, status: e.target.value })} className='w-full bg-slate-950 border border-slate-800 rounded-lg py-2 px-3 text-sm text-slate-200 focus:outline-none focus:border-indigo-500'>
          ...
          <AnimatePresence>
            {showDeleteModal && (
              <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm'>
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className='bg-slate-900 border border-slate-800 rounded-xl max-w-sm w-full p-6 text-center shadow-2xl'>
