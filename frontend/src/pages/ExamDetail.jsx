...
import { 
  ...
  FiChevronRight, FiExternalLink, FiArrowLeft
} from 'react-icons/fi';
...
function ExamDetail() {
  ...
  const fetchExamData = async () => {
    try {
      setLoading(true);
      setError('');
      console.log('🔍 Fetching exam with slug:', slug);
  ...
  if (error || !examData) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto p-8 bg-gray-900/50 rounded-2xl border border-gray-800">
          ...
          <p className="text-gray-400 mb-6">
            {error || `No details found for exam: "${slug}"`}
          </p>
          <Link 
            to="/exams" 
  ...
  const career = parseJSON(examData.career_progression);
  const related = parseJSON(examData.related_exams);
          ...
          <div className="flex flex-col md:flex-row items-start gap-6">
            <div className="flex-1">
              <h1 className="text-4xl md:text-5xl font-bold text-white mb-2">
                {examData.exam_title || examData.exam_name}
              </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {eligibility.note && (
                    <p className="text-xs text-gray-500 mt-2">{eligibility.note}</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {age.maximum_age?.note && (
                    <p className="text-xs text-gray-500 mt-2">{age.maximum_age.note}</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {application.application_fee?.note && (
                    <p className="text-xs text-gray-500 mt-2">{application.application_fee.note}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {/* Key Dates */}
            {dates && (dates.notification_release || dates.application_start) && (
              <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
        ...
        {activeTab === 'syllabus' && (
          <div className="space-y-8">
                  ...
                  {pattern.stage_1.paper_details && pattern.stage_1.paper_details.length > 0 && (
                    <div className="mt-4 space-y-2">
                      {pattern.stage_1.paper_details.map((detail, idx) => (
                        <div key={idx} className="p-2 bg-gray-800/30 rounded-lg text-sm">
                          {detail}
                        </div>
                      ))}
                    </div>
            ...
            {/* Syllabus */}
            {syllabus.stage_1 && syllabus.stage_1.topics && (
              <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
                <h2 className="text-xl font-bold text-white mb-4">Detailed Syllabus</h2>
                <div className="space-y-4">
                  {Object.entries(syllabus.stage_1.topics).map(([subject, topics]) => (
                    <div key={subject} className="p-4 bg-gray-800/30 rounded-xl">
                      <h3 className="font-semibold text-blue-400 mb-2">{subject}</h3>
                      <p className="text-gray-300 text-sm">{topics}</p>
                    </div>
                  ))}
                </div>
              </div>
